...
from genie3.genie3 import run, partition_data, calculate_importances, rank_genes_by_importance
...
class TestGenie3:
    ...
    def test_rank_genes_by_importance(self):
        ...
        assert set(predicted_network.columns) == {"transcription_factor", "target_gene", "importance"}
        
        # Check that the network is sorted by importance (descending)
        assert predicted_network["importance"].iloc[0] >= predicted_network["importance"].iloc[1]
        assert predicted_network["importance"].iloc[1] >= predicted_network["importance"].iloc[2]
    ...
    def test_calculate_importances_integration(self, sample_dataset, regressor_config):
        ...
        gene_expressions = sample_dataset.gene_expressions.values
        tf_indices = list(sample_dataset._transcription_factor_indices)
    ...
    def test_run_integration(self, sample_dataset, regressor_config):
        ...
        assert set(result.columns) == {"transcription_factor", "target_gene", "importance"}
