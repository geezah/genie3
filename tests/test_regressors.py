...
class TestRegressorFactory:
    def test_get_regressor(self):
        ...
        assert (
            RegressorFactory.get("RandomForestRegressor")
            == RandomForestRegressor
        )
...
class TestConfigurationFactory:
    ...
    def test_configuration_validity(self, sample_data):
        ...
        for regressor_class in RegressorFactory._regressors.values():
            ...
            assert hasattr(regressor, "feature_importances_")
            assert regressor.feature_importances_.shape == (X.shape[1],)
            assert np.isclose(np.sum(regressor.feature_importances_), 1.0)
...
class TestRegressors:
    def test_regressor_protocol_compliance(self):
        ...
        for regressor_class in RegressorFactory._regressors.values():
            ...
            # Check that the feature_importances_ property raises an error when not fitted
            with pytest.raises(
    ...
    @pytest.mark.parametrize(
        "regressor_class", [RandomForestRegressor, ExtraTreesRegressor]
    )
